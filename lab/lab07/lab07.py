...
def make_glookup(class_assignments):
    ...
    "*** YOUR CODE HERE ***"
    current_score = {}
    def glookup(item, score):
        nonlocal current_score
        current_score[item] = score
        gained_score, planed_socre = 0,0
        for i in list(current_score.keys()):
            gained_score, planed_socre = gained_score + current_score[i], planed_socre + class_assignments[i]
        return gained_score / planed_socre 
...
def make_advanced_counter_maker():
    ...
    def make_advanced_counter():
        count_num = 0
        def advanced_counter(assignment):
            nonlocal global_count, count_num
            "*** YOUR CODE HERE ***"
            if assignment == 'count':
                count_num = count_num + 1
                return count_num 
            elif assignment == 'reset':
                count_num = 0
            elif assignment == 'global-count':
                global_count += 1
                return global_count
            elif assignment == 'global-reset':
                global_count = 0
