...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        for elem in player.deck.cards:
            elem.attack = elem.attack + other_card.attack
            elem.defense = elem.defense + other_card.defense
        for elem in opponent.deck.cards[:]:
            if elem.attack == other_card.attack or elem.defense == other_card.defense:
                opponent.deck.cards.remove(elem)
