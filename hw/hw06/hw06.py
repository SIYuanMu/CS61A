...
def is_bst(t):
    ...
    branch = [b for b in t.branches]

    if len(branch) > 2:
        return False
    elif len(branch) == 2:
        if bst_max(branch[0]) <= t.label and bst_min(branch[1]) > t.label:
            return is_bst(branch[0]) and is_bst(branch[1])
        else:
            return False
    else:
        """
        if bst_max(branch[0]) > t.label and bst_min(branch[0]) <= t.label:
            return False
        elif bst_max(branch[0]) > t.label or bst_min(branch[0]) <= t.label:
            return is_bst(branch[0])
        """
        if branch[0].label <= t.label and bst_max(branch[0]) <= t.label:
            return is_bst(branch[0])
        elif branch[0].label > t.label and bst_min(branch[0]) > t.label:
            return is_bst(branch[0])
        else:
            return False
...
def path_yielder(t, value):
    ...
    if t.label == value:
        ...
        for b in t.branches:
            for elem in path_yielder(b, value):
                "*** YOUR CODE HERE ***"
                yield [t.label].extend(elem)
    """
    """
    if t.is_leaf():
        if t.label != value:
