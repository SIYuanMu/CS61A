...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, input_password):
        nonlocal incorrect_passwords, balance
        if len(incorrect_passwords) == 3:
            incorrect_str = '[' + '\'' + incorrect_passwords[0] + '\', \'' + incorrect_passwords[1] + '\', \'' + incorrect_passwords[2] + '\']'
            return 'Too many incorrect attempts. Attempts: ' + incorrect_str
        else:
            if input_password == password:
                ...
                incorrect_passwords += [input_password]
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    what_out = withdraw(0, old_pass)
    if type(what_out) == str:
        return what_out
    else:
...
def remainders_generator(m):
    ...
    def remainders(i):
        nums = naturals()
        yield from filter(lambda x: x % m == i, nums)
