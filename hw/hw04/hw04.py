...
def balanced(m):
    ...
    total_left = length(left(m)) * total_weight(end(left(m)))
    total_right = length(right(m)) * total_weight(end(right(m)))
...
def has_path(t, phrase):
    ...
    "*** YOUR CODE HERE ***"
    temp_pharse, nods = phrase, [t]
    for i in range(len(phrase)):
        labels = [label(b) for b in nods]
        if temp_pharse[0] not in labels:
            return False
        nods = sum([branches(b) for b in nods], [])
        temp_pharse = temp_pharse[1:]
    return True
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert 1/upper_bound(y) < 1/lower_bound(y), 'This interval can not be used as divisor'
